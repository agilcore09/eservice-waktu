  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Input Biaya Servis</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
            <form method="POST" action="{{ route('store-kerusakan')}}">
            {{csrf_field()}}
                        ...
                        <div class="card-body">
                            <table id="example" class="table table-bordered table-striped">
                                ...
                                <tbody>
                                    ...
                                    {{-- <td widtd="10px">{{ $v->id}}</td> --}}
                                    <td>Rp.{{ number_format($v->biaya)}}</td>
                                   

                                    <td width="13%">

                                     <a href="{{ route('edit_kerusakan',$v->id)}}">
                                    <button class="btn btn-info btn-xs" id="add">
                                        <i class="fa fa-edit"></i>

                                </button></a> <a onclick="return confirm('anda yakin ingin menghpus {{$v->kerusakan}}?')" href="{{ route('delete_kerusakan',$v->id)}}" class="delete-confirm">
